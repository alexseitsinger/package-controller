...
def make_changelog(
    changelog_output="CHANGELOG.md", changelog_type="angular", changelog_style="angular"
):
    assert_repository()
    assert_remotes()
    assert_which("git-changelog")
    run(
        "git-changelog . -o {changelog_output} -t {changelog_type} -s {changelog_style}".format(
